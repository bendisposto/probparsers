...
import de.prob.parser.antlr.ScopeChecker.ScopeCheckerVisitorException;
...
public class MachineAnalyser {
    ...
    final LinkedHashMap<String, BOperationContext> operationsDeclarations = new LinkedHashMap<>();
...
//    }
    ...
    class DeclarationFinder extends BParserBaseVisitor<Void> {
        DeclarationFinder() {
...
//        }
		...
		@Override public Void visitInitialisationClause(BParser.InitialisationClauseContext ctx) {
...
//    }
    ...
    class MachineScopeChecker extends ScopeChecker {
        ...
        void check() throws ParseErrorException {
            ...
            for (Entry<String, BOperationContext> entry : MachineAnalyser.this.operationsDeclarations.entrySet()) {
                scopeTable.clear();
                scopeTable.add(MachineAnalyser.this.setsDeclarations);
                scopeTable.add(MachineAnalyser.this.constantsDeclarations);
                scopeTable.add(MachineAnalyser.this.variablesDeclarations);
                scopeTable.add(MachineAnalyser.this.definitionsDeclarations);
                BOperationContext operation = entry.getValue();
                if (operation.output != null) {
                    LinkedHashMap<String, TerminalNode> outputMap = new LinkedHashMap<>();
                    for (TerminalNode terminalNode : operation.output.IDENTIFIER()) {
                ...
                if (operation.parameters != null) {
                    ...
                    for (TerminalNode terminalNode : operation.parameters.IDENTIFIER()) {
