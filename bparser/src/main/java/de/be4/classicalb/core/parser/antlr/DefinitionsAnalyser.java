...
import files.BParser.ComposedIdentifierContext;
import files.BParser.ExpressionContext;
import files.BParser.ExpressionFunctionCallContext;
import files.BParser.ExpressionIdentifierContext;
import files.BParser.Expression_in_parContext;
import files.BParser.Expression_listContext;
import files.BParser.SubstitutionIdentifierCallContext;
	...
	class DefinitionUsage extends BParserBaseVisitor<Void> {
		...
		private void addDefinitionTypeUsage(final String defName, final DefinitionType type,
			...
			if (definitionTypes.get(defName) != DefinitionType.UNKNOWN_TYPE && definitionTypes.get(defName) != type) {
				...
				BParseException exception = new BParseException(null, sourcecodeRange, msg);
