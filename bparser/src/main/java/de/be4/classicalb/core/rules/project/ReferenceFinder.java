...
public class ReferenceFinder extends DepthFirstAdapter {
	...
	private final LinkedHashMap<String, Reference> referncesTable;
	...
	public void findReferencedMachines() throws BException {
		try {
			...
			if (mainFile != null) {
				throw new BException(mainFile.getAbsolutePath(), e.getException());
			} else {
				throw new BException("UnkownFile", e.getException());
			}
		}
	}
	...
	public Hashtable<String, Reference> getReferencesTable() {
		return new Hashtable<>(referncesTable);
	}

	public List<Reference> getReferences() {
		ArrayList<Reference> list = new ArrayList<>();
		for (Entry<String, Reference> entry : referncesTable.entrySet()) {
	...
	public void caseAMachineReference(AMachineReference node) {
		...
		if (node.parent() instanceof AFileMachineReference) {
			...
			Reference ref;
			try {
				ref = new Reference(name, node, file);
			...
			try {
				machineReference = new Reference(mainFile.getParentFile(), name, node, this.pathList);
			} catch (CheckException e) {
				throw new VisitorException(e);
			}
	...
	public void caseAReferencesMachineClause(AReferencesMachineClause node) {
		registerMachineNames(node.getIdentifiers());
	}
	...
	private void registerMachineNames(List<PExpression> referencedMachineList) {
		for (PExpression machineExpression : referencedMachineList) {
			if (machineExpression instanceof AIdentifierExpression) {
				...
				try {
					Reference machineReference = new Reference(mainFile.getParentFile(), name, identifier, pathList);
					if (this.filePathTable.containsKey(name)) {
						machineReference.setDirectoryPath(filePathTable.get(name));
					}
					referncesTable.put(name, machineReference);
				...
				Reference machineReference;
				try {
					machineReference = new Reference(name, identifier, file);
					referncesTable.put(name, machineReference);
				} catch (CheckException e) {
					throw new VisitorException(e);
				}
